...
|---|---|---|
| 中央技能库 + 软链接同步 | ✅ 中央库 + symlink/copy 两种模式 | ✅ 本体库 + 相对软链（**仅 symlink**） |
| 多平台支持 | ✅ 15+ agent | ✅ 内置 6 + 自定义 + 自动检测 |
| 场景/预设（Preset） | ✅ **按场景成组，一键激活/停用整组** | ❌ 无 |
| 项目级 skills 管理 | ✅ 项目工作区，双向同步 | ❌ 只管用户级目录 |
| Git 备份/多机同步 | ✅ 自动 pull/commit/push + 快照标签回滚 | ❌ 无 |
| 技能市场浏览 | ✅ 内置 skills.sh 市场 + AI 语义搜索 | ❌ 只能装已知 URL |
| 体检/健康检查 | ⚠️ 基础 | ✅ **Doctor 五项检查 + 一键修复**（GUI 独有） |
| 安全扫描 | ❌（Skills Manager Client 有） | ❌ 无 |
...
1. **Doctor 体检 + 一键修复**：断链、frontmatter 缺失、重名、引用缺失、体积异常——GUI 竞品里没有等价物（仅 Rust 版 skills-cli 有 `skills doctor` 命令，但无修复能力）
...
| 优先级 | 特性 | 来源 | 落地思路 |
|---|---|---|---|
| ⭐⭐⭐ | **场景/预设（Presets）** | Skills Manager | 命名一组 skill，一键将整组启用/停用到指定平台。与现有"收藏夹"机制互补，复用 batchEnable/batchDisable |
| ⭐⭐⭐ | **技能市场浏览** | Skills Manager / skillfish | 内置 skills.sh 目录浏览与搜索，一键安装。安装管线已具备，只差发现层 |
| ⭐⭐ | **Manifest 导出/导入** | skillfish bundle | 把当前 skills 清单导出为 JSON，换机/团队共享时一键装回。与 lock 文件天然契合 |
| ⭐⭐ | **安全扫描** | Skills Manager Client | 扩展 Doctor：扫描危险 shell 模式、网络请求、凭据读取，给安全评分 |
| ⭐⭐ | **项目级 skills** | Skills Manager 项目工作区 | 扫描 `<project>/.claude/skills` 等，支持项目级启用 |
| ⭐ | **Git 备份/多机同步** | Skills Manager | 本体库本身就是目录，可一键 git init + 推送到私有仓库 |
| ⭐ | **copy 同步模式** | Skills Manager / skills CLI `--copy` | SkillOps.enable 增加复制选项，适配不支持软链的场景 |
| ⭐ | **版本固定安装 @tag/@SHA** | gh skill | install 支持 `repo@tag` 语法 |
